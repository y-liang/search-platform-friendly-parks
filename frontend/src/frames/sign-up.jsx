...
const SignUp = () => {
    ...
    useEffect(() => {
        // if (auth.token) {
        //     setReminder('You have already logged in, if you wish to sign up for another account, please log out first.');
    ...
    const handleSubmit = async (event) => {
        ...
        const result = await actAccount.signUp(fields); // result - data - response
        ...
        <div className={styles.wrapper}>
            ...
            <form>
                ...
                <div className={styles.row}>
                    ...
                    <Link to='/account/login' state={{ from: location.pathname }} className={styles.instead}>Log in instead?</Link>
                </div>

                <div className={styles.row}>
                    <span className={styles.message}>{message ? message : null}</span>
                    <button type='submit' onClick={handleSubmit} className={styles.submit}>Sign Up</button>
                </div>
