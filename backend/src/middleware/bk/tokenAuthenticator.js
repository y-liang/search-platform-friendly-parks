...
const tokenAuthenticator = async (req, res, next) => {
    ...
    if (token == 'null') {
        req.tokenMissing = true;  // some routes need to make sure not token is present
    ...
    try {
        ...
        req.tokenEmail = decoded.email; // in database this email could be nonexistent or deactivated or active 
        req.tokenVia = decoded.via; // how the token was generated
        return next();
